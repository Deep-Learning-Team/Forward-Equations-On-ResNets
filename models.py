...
class TDRNN(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        out = LReLU(-self.h2(out) + torch.matmul(out,
                                                 self.h2.weight.t())) + out
        out = LReLU(-self.h3(out) + torch.matmul(out,
                                                 self.h3.weight.t())) + out
        out = LReLU(-self.h4(out) + torch.matmul(out,
                                                 self.h4.weight.t())) + out
        out = self.output(out)
...
class ODRNN(torch.nn.Module):
    ...
    def __init__(self, input_size):
        ...
        self.H1_DROPOUT = 0.9
        self.LR = 0.1
        self.h1 = nn.Linear(int(self.INPUT_SIZE / 2),
                            int(self.H1_SIZE / 2), bias=True)
        self.h2 = nn.Linear(int(self.H1_SIZE / 2),
                            int(self.H2_SIZE / 2), bias=False)
        self.h3 = nn.Linear(int(self.H2_SIZE / 2),
                            int(self.H3_SIZE / 2), bias=False)
        self.h4 = nn.Linear(int(self.H3_SIZE / 2),
                            int(self.H4_SIZE / 2), bias=False)
...
class SORNN(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        # print(torch.matmul(x, self.h1.weight.t()).shape)
        out_1 = self.drop(
            LReLU(-self.h1(x) + torch.matmul(x, self.h1.weight.t()))) + x
        out_2 = - torch.matmul(LReLU(self.h2(x)),
                               self.h2.weight.t()) + 2 * out_1 - x
        out_3 = - torch.matmul(LReLU(self.h3(x)),
                               self.h3.weight.t()) + 2 * out_2 - out_1
        out_4 = - torch.matmul(LReLU(self.h4(x)),
                               self.h4.weight.t()) + 2 * out_3 - out_2
        out = LReLU(-self.h5(out_4) + torch.matmul(out_4,
                                                   self.h5.weight.t())) + out_4
        out = self.output(out)
